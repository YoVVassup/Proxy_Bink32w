...
255 个测试分布在 31 个测试套件中，覆盖所有核心模块：

| 模块 | 测试数 | 覆盖率 |
|------|--------|--------|
| config.cpp（CRC32、.mix 解析器、.bik 头、.wav 解码器、配置解析器） | 78 | 100% |
| binkw32_proxy.cpp（TrackVideo、UntrackVideo、FindVideo、缩放、DLL 生命周期、ExtractFileName） | 47 | 100% |
...
│   ├── binkw32_1.9u.dll
│   └── ...
├── tests/                   # Google Test 套件（255 个测试，31 个测试套件）
